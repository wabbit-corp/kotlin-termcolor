...
/**
 * A string trie for quickly looking up values of type [[T]]
 * using string-keys.
 */
internal class Trie<T : Any>(strings: List<Pair<String, T>>) {
    ...
    init {
        ...
        if (continued > 0) {
            ...
            this.arr = continuations.map {
                if (it.isNotEmpty()) Trie(it)
                else null
            }.toTypedArray()
        } else {
            this.arr = emptyArray()
    ...
    operator fun get(c: Char): Trie<T>? {
        if (c > max || c < min) return null
        else return this.arr[c - min]
    }
}
