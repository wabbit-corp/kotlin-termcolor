...
/**
 * Represents one or more [[AnsiStyle]]s, that can be passed around
 * as a set or combined with other sets of [[AnsiStyle]]s.
 */
sealed interface AnsiStyle {
    /**
     * Which bits of the [[EncodedAnsiStyle]] integer these [[AnsiStyle]] will
    ...
    /**
     * Apply the current [[AnsiStyle]] to the [[EncodedAnsiStyle]] integer,
    ...
    /**
     * Combine this [[AnsiStyle]] with other [[AnsiStyle]]s, returning one
     * which when applied is equivalent to applying this one and then the `other`
     * one in series.
     */
    operator fun plus(other: AnsiStyle): AnsiStyle = when (this) {
        is Combined -> when (other) {
    ...
    companion object {
         ...
         */
        @JvmStatic val Reset: AnsiStyle = EscapeAnsiStyle(AnsiCodes.RESET, Long.MAX_VALUE.toLong(), 0, "Escape")
         ...
         * unnecessarily. Reads its category listing from a fast Array version of
         * [[categories]] and writes it's output to a mutable `StringBuilder`
         */
        fun emitAnsiCodes0(currentState: EncodedAnsiStyle, nextState: EncodedAnsiStyle, output: StringBuilder, categoryArray: Array<Category>) {
        ...
        operator fun invoke(attrs: List<Single>): AnsiStyle {
            ...
            else Combined(resetMask, applyMask, output.reversed())
        }
    }

    private data class Combined internal constructor (
        override val resetMask: Long, override val applyMask: Long,
     ...
     * are not widely/directly supported by terminals and so Str supports them
     * by rendering a hard [[AnsiStyle.Reset]] and then re-rendering other [[AnsiStyle]]s that are
     * active.
     *
     ...
     */
    class EscapeAnsiStyle internal constructor (
     ...
     */
    class ResetAnsiStyle internal constructor(
        override val resetMask: Long,
        override val applyMask: Long,
        override val name: String
    ) : Single {
    ...
    /**
     * Represents a set of [[AnsiStyle]]s all occupying the same bit-space
     * in the state `Int`
     */
    sealed class Category(val offset: Int, val width: Int, val catName: String) {
        ...
        open internal val lookupTableWidth = 1 shl width
    ...
    /**
     * * Color encoded on 25 bit as follow :
     * 0 : reset value
     * 1 - 16 : 3 bit colors
     * 17 - 272 : 8 bit colors
     * 273 - 16 777 388 : 24 bit colors
     */
    abstract class ColorCategory(offset: Int, width: Int, val colorCode: Int, catName: String) : Category(offset, width, catName) {
        /**
        ...
        override internal val lookupTableWidth = 273
    }
}
